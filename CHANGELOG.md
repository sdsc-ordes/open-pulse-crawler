...
- Nginx reverse-proxy container (`nginx/Dockerfile` + `nginx/nginx.conf`): routes `/api/*` to FastAPI (port 8000) and `/` to Streamlit (port 8501) with WebSocket upgrade support.
