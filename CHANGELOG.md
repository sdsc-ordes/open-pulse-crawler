...
- `--max-contributors` per-repo contributor cap (CLI flag, REST API field on `POST /api/v1/crawl`, Streamlit GUI input under "Performance & filtering", and constructor argument on `GitHubCrawler`). It is a **take-up-to-N limit**: at most N contributors are recorded and queued per repo, and a repo with more contributors is truncated to its top N — it still contributes, it is never skipped to zero. Unset, the crawler uses a built-in default cap (10). Like every cap in the project, `max_contributors` means "take up to N", never "skip to 0".
- `RepoModel.contributor_count` field — the repo's total contributor count, recorded as metadata (the `contributors` list itself is capped at `max_contributors`). Captured the first time a repo is fetched (one cheap `per_page=1` request via `repo.get_contributors().totalCount`) and cached.
