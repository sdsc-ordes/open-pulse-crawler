...
- GraphQL-backed crawl endpoint:
  ...
  - **Token rotation:** a comma-separated `GITHUB_TOKEN` now rotates across all tokens, matching the REST client. When the active token's GraphQL point budget is spent the client switches to the next token; only once every token has been tried and exhausted does it sleep until the window resets. Previously the GraphQL client accepted the token list but only ever used the first one. `token_switches` / `rate_limit_waits` are reported in `get_stats()`.
