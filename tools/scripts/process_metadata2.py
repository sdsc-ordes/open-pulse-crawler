...
class MetadataProcessor:
    ...
    def _get_api_endpoint(self, item: str, item_type: str) -> str:
        ...
        if item_type == "user":
            return f"{self.base_url}/v1/user/llm/json/{github_url}?force_refresh=true"
        elif item_type == "repo":
            return f"{self.base_url}/v1/repository/llm/json/{github_url}?force_refresh=true"
        elif item_type == "org":
            return f"{self.base_url}/v1/org/llm/json/{github_url}?force_refresh=true"
        else:
            raise ValueError(f"Unknown item type: {item_type}")
