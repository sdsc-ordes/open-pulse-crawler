...
- **`platforms/github/client.py`** / **`platforms/github/graphql.py`**: GitHub API wrappers (REST + GraphQL), re-exported from the `platforms.github` package as `GitHubClient` / `GitHubGraphQLClient`. Public methods (`get_user`, `get_organization`, `get_repository`, ...) take canonical URLs; logins / `owner/repo` are extracted internally for the GitHub call. Cache keys are URL-based.
